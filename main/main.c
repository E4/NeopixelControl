...
static void move_chasers() {
  ...
  for(i=0;i<chaser_count;i++) {
    if(frame%chaser_data[i].position_delay==0) {
      chaser_data[i].position_offset = (uint16_t)positive_mod((int)chaser_data[i].position_offset + chaser_data[i].position_speed, chaser_data[i].range_length);
    }
    ...
    if(chaser_data[i].repeat) {
      repeats = chaser_data[i].range_length/chaser_data[i].repeat;
      for(r=0;r<repeats;r++) {
        chaser_pixel[r].index = ((chaser_data[i].position_offset + chaser_data[i].repeat * r) % chaser_data[i].range_length) + chaser_data[i].range_offset;
        chaser_pixel[r].rgb = chaser_color;
      }
      neopixel_SetPixel(neopixel, chaser_pixel, r);
    } else {
      chaser_pixel[0].index = chaser_data[i].position_offset + chaser_data[i].range_offset;
